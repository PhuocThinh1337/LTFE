...
const VoucherPage: React.FC = () => {
  ...
  const handleSaveVoucher = (voucherId: number) => {
    const isSaved = savedVouchers.includes(voucherId);
    let newSavedVouchers;
    
    if (isSaved) {
      newSavedVouchers = savedVouchers.filter(id => id !== voucherId);
    } else {
      newSavedVouchers = [...savedVouchers, voucherId];
    }
              ...
              {activeVouchers.map((voucher) => (
                <div key={voucher.id} className="np-voucher-card">
                  ...
                  <div className="np-voucher-actions">
                    <button 
                      className={`np-save-voucher-btn ${savedVouchers.includes(voucher.id) ? 'saved' : ''}`}
