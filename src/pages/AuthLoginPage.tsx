...
function AuthLoginPage(): React.JSX.Element {
  ...
  const handleGoogleSuccess = async (googleData: { email: string; name: string; picture?: string }) => {
    ...
    try {
      ...
      setErrors({
        submit: error instanceof Error ? error.message : 'Đăng nhập Google thất bại. Vui lòng thử lại.'
      });
      setIsGoogleModalOpen(false);
    } finally {
